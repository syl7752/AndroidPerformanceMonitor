...
public class LinpackLoop {
	...
	public String run_benchmark() {
		...
		normx = 0.0;
		for (i = 0; i < n; i++) {
			resid = (resid > Math.abs((double) b[i])) ? resid : Math
					.abs((double) b[i]);
			normx = (normx > Math.abs((double) x[i])) ? normx : Math
					.abs((double) x[i]);
		}
...
//				+ "    Precision: " + eps_result);
	...
	double matgen(double a[][], int lda, int n, double b[]) {
		 ...
		 * order. --dmd 3/3/97 --added to this source jch 3/7/97
		 */
		for (i = 0; i < n; i++) {
			for (j = 0; j < n; j++) {
		...
		if ((n > 0) && (da != 0)) {
			if (incx != 1 || incy != 1) {
				...
				for (i = 0; i < _n; i += 4) {
					...
					dy[i + 1 + dy_off] = dy[i + 1 + dy_off] + da
							* dx[i + 1 + dx_off];
					dy[i + 2 + dy_off] = dy[i + 2 + dy_off] + da
							* dx[i + 2 + dx_off];
					dy[i + 3 + dy_off] = dy[i + 3 + dy_off] + da
							* dx[i + 3 + dx_off];
				}
				for (i = _n; i < n; i++)
					dy[i + dy_off] = dy[i + dy_off] + da * dx[i + dx_off];
